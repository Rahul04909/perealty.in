<?php
/**
 * Admin Enquiries Management Portal
 * Prime Edge Realty
 */
...
// Generate CSRF Token for enquiries actions
if (empty($_SESSION['enquiries_csrf_token'])) {
    $_SESSION['enquiries_csrf_token'] = bin2hex(random_bytes(32));
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (empty($_POST['csrf_token']) || !hash_equals($_SESSION['enquiries_csrf_token'], $_POST['csrf_token'])) {
...
try {
    ...
    $enquiries = [];
    $error = 'Failed to load enquiries: ' . $e->getMessage();
}
...
<div class="container-fluid text-dark">
    ...
    <div class="row mb-3">
        <div class="col-12">
            <h1 class="h3 text-dark font-weight-bold">Client Enquiries</h1>
            <p class="text-muted">Manage incoming property investment leads and follow up on client requests.</p>
        </div>
    </div>
                ...
                <table class="table table-hover table-striped mb-0 align-middle" style="font-size: 0.9rem;">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th style="padding: 12px 15px; width: 180px;">Project Title</th>
                            <th style="padding: 12px 15px; width: 150px;">Client Info</th>
                    ...
                    <tbody>
                        <?php if (empty($enquiries)): ?>
                            <tr>
                                <td colspan="6" class="text-center py-4 text-muted">No enquiries registered yet. Submissions from property pages will show up here.</td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($enquiries as $enq): ?>
                                ...
                                <tr>
                                    ...
                                    <td style="padding: 12px 15px;">
                                        <span class="font-weight-bold text-dark d-block"><?php echo htmlspecialchars($enq['project_title']); ?></span>
                                        <small class="text-muted">ID Ref: <?php echo $enq['project_id'] ? htmlspecialchars($enq['project_id']) : 'General'; ?></small>
                                    </td>
...
<form id="action-form" action="enquiries.php" method="post" style="display:none;">
    <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['enquiries_csrf_token'] ?? ''); ?>">
...
function confirmDelete(id, clientName) {
    Swal.fire({
        title: 'Are you sure?',
        text: 'Do you want to permanently delete ' + clientName + '\'s enquiry?',
        icon: 'warning',
